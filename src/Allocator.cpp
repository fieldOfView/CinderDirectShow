            ...
            /* [out][in] */ DWORD *lpNumBuffers)
{
	...
	mCiSurface = ci::Surface( (int)lpAllocInfo->dwWidth, (int)lpAllocInfo->dwHeight, true, ci::SurfaceChannelOrder::BGRA );

    return S_OK;//m_scene.Init(m_D3DDev);
}
...
HRESULT CAllocator::PresentHelper(VMR9PresentationInfo *lpPresInfo)
{
    ...
    {   
		...
		FAIL_RET( MakeCinderSurface( m_D3DDev, m_privateTexture ));
    }
         ...
         // all we need to do is to get them from the surface
    {
		...
		FAIL_RET( MakeCinderSurface( m_D3DDev, texture ));
    }
...
HRESULT CAllocator::MakeCinderSurface( IDirect3DDevice9* aD3dDevice, IDirect3DTexture9* aTexture ) 
{
	if (!mCiSurface)
		return -1;

	// Based on http://www.gamedev.net/topic/171600-getting-texture-data/

	IDirect3DSurface9* pD3dSurface = NULL;
	aTexture->GetSurfaceLevel( 0, &pD3dSurface );
	D3DSURFACE_DESC pD3dSurfaceDesc;
	pD3dSurface->GetDesc( &pD3dSurfaceDesc );
	
	IDirect3DSurface9* pReadSurface = NULL;	
	aD3dDevice->CreateOffscreenPlainSurface( pD3dSurfaceDesc.Width, pD3dSurfaceDesc.Height, pD3dSurfaceDesc.Format, D3DPOOL_SYSTEMMEM, &pReadSurface, 0);

	aD3dDevice->GetRenderTargetData( pD3dSurface, pReadSurface );

	D3DLOCKED_RECT sLockedRect;

	if( SUCCEEDED( pReadSurface->LockRect( &sLockedRect, NULL, 0) ) )
	{
		int iStride = 0;
		switch(pD3dSurfaceDesc.Format)
		{
			case D3DFMT_L8 :
				iStride = 1;
				break;
		...
		memcpy( mCiSurface.getData(), sLockedRect.pBits, iStride * pD3dSurfaceDesc.Width * pD3dSurfaceDesc.Height );
		_signal( this );

	} 

	pReadSurface->UnlockRect();
	pReadSurface->Release();
	pD3dSurface->Release();
